...
void ConveyorBeltPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  this->model = boost::static_pointer_cast<physics::Model>(this->link->GetModel());
  if (!this->model)
  {
    gzerr << "Model not found" << std::endl;
    return;
  }

  this->InitPoseBelt = this->model->GetWorldPose();
  this->Init_euler_rot = this->InitPoseBelt.rot.GetAsEuler();

  gzdbg << "Init PoseBelt: [" << this->InitPoseBelt.pos.x  << "," << this->InitPoseBelt.pos.y << "," << this->InitPoseBelt.pos.z << "]" << std::endl;
  gzdbg << "Init BeltRPY: [" << this->Init_euler_rot.x  << "," << this->Init_euler_rot.y << "," << this->Init_euler_rot.z << "]" << std::endl;
...
void ConveyorBeltPlugin::OnUpdate()
{
  ...
  if (this->joint->GetAngle(0) >= this->limit)
  {
    ...
    */
    float beltpos = 0.88122;


    const math::Pose childLinkPose(this->InitPoseBelt.pos.x,this->InitPoseBelt.pos.y, this->InitPoseBelt.pos.z+beltpos, this->Init_euler_rot.x,this->Init_euler_rot.y,this->Init_euler_rot.z);
    const math::Pose newChildLinkPose(this->InitPoseBelt.pos.x - 1.0,this->InitPoseBelt.pos.y, this->InitPoseBelt.pos.z+beltpos, this->Init_euler_rot.x,this->Init_euler_rot.y,this->Init_euler_rot.z);
    this->link->MoveFrame(childLinkPose, newChildLinkPose);

    // We Reset this because the Hack doesnt allow different positions of the conveyor.
